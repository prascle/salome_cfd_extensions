...
2. Create a build directory:
  ...
  rm -rf ${MOD_SATURNE_8_BUILD}
  mkdir ${MOD_SATURNE_8_BUILD}
  cd ${MOD_SATURNE_8_BUILD}
...
3. Configure the build procedure:
  ...
  cmake \
  -S"${MOD_SATURNE_8_SRC}" \
  -B"${MOD_SATURNE_8_BUILD}" \
  -G"Unix Makefiles" \
  -DCMAKE_INSTALL_PREFIX:PATH="${MOD_SATURNE_8_INSTALL}" \
  -DSALOME_BUILD_DOC="TRUE"
 

   Note: by default (if CMAKE_INSTALL_PREFIX option is not given), SALOME SATURNE8
...
4. Build and install:
   ...
   This will install SALOME SATURNE8 module to the <installation_directory>
